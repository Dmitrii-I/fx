add_offset_to_timestamp <- function(files, offset_in_sec = 0, header = TRUE)
{
    results <- vector(length = length(files), mode = "logical")

    for (i in 1:length(files))
    {
        ticks <- read.csv(files[i], header = header)
        ...
        # stripping the unnecessary decimals instead of rounding.  We will 
        # round the milliseconds ourselves.
        ...
        write.table(ticks, file = files[i], quote = FALSE, eol = "\n", 
